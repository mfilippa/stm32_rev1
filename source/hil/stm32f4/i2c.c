...
typedef enum i2c_sm_state_enum {
    I2C_SM_STATE_IDLE,
    I2C_SM_STATE_TX_START,
    I2C_SM_STATE_TX_ADDRESS,
    I2C_SM_STATE_TX_DATA,
    I2C_SM_STATE_RX_START,
    I2C_SM_STATE_RX_ADDRESS,
    I2C_SM_STATE_RX_DATA,
    I2C_SM_STATE_STOP,
    I2C_SM_STATE_WAIT_BUSY,
    I2C_SM_STATE_WAIT_START,
    I2C_SM_STATE_WAIT_TX_ADDRESS,
    I2C_SM_STATE_WAIT_TX_DATA,
    I2C_SM_STATE_WAIT_RX_ADDRESS,
    I2C_SM_STATE_WAIT_RX_DATA,
} i2c_sm_state_t;

// module structure
struct i2c_struct {
    // state machine vars
    i2c_sm_state_t state;
    i2c_sm_state_t next_state;
...
void i2c_init(void){
    ...
    // init state machine
    i2c.state = i2c.next_state = I2C_SM_STATE_IDLE;
...
void i2c_write(uint32_t address, uint32_t * data, uint32_t nr_bytes,
        void (*callback)(void)){
    // save data
    if((nr_bytes!=0)&&(i2c.state == I2C_SM_STATE_IDLE)){
...
i2c_state_t i2c_get_state(void){
    if (i2c.state==I2C_SM_STATE_IDLE) return I2C_STATE_IDLE;
    else return I2C_STATE_BUSY;
}
...
void i2c_step(void){
    ...
    case I2C_SM_STATE_IDLE:
        ...
        if (i2c.tx_size!=0){
            ...
            // start TX
            i2c.next_state = I2C_SM_STATE_TX_START;
            i2c.state = I2C_SM_STATE_WAIT_BUSY;
        }
        // EVENT: RX requested
            ...
            I2C_AcknowledgeConfig(I2C2, ENABLE);
            // start RX
            i2c.next_state = I2C_SM_STATE_RX_START;
            i2c.state = I2C_SM_STATE_WAIT_BUSY;
        }
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_TX_START:
        // generate start condition
        I2C_GenerateSTART(I2C2, ENABLE);
        i2c.next_state = I2C_SM_STATE_TX_ADDRESS;
        i2c.state = I2C_SM_STATE_WAIT_START;
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_RX_START:
        // generate start condition
        I2C_GenerateSTART(I2C2, ENABLE);
        i2c.next_state = I2C_SM_STATE_RX_ADDRESS;
        i2c.state = I2C_SM_STATE_WAIT_START;
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_TX_ADDRESS:
        // send address and request write
        I2C_Send7bitAddress(I2C2, i2c.tx_address, I2C_Direction_Transmitter);
        i2c.tx_index = 0;
        i2c.next_state = I2C_SM_STATE_TX_DATA;
        i2c.state = I2C_SM_STATE_WAIT_TX_ADDRESS;
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_RX_ADDRESS:
        // send address and request read
        I2C_Send7bitAddress(I2C2, i2c.rx_address, I2C_Direction_Receiver);
        i2c.rx_index = 0;
        i2c.next_state = I2C_SM_STATE_RX_DATA;
        i2c.state = I2C_SM_STATE_WAIT_RX_ADDRESS;
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_TX_DATA:
        // send data
        ...
        // no more data to send
            ...
            // else stop comm
                ...
                i2c.next_state = I2C_SM_STATE_STOP;
            }
        }
        i2c.state = I2C_SM_STATE_WAIT_TX_DATA;
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_RX_DATA:
        ...
        i2c.state = I2C_SM_STATE_WAIT_RX_DATA;
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_STOP:
        I2C_GenerateSTOP(I2C2, ENABLE);
        i2c.state = I2C_SM_STATE_IDLE;
        // callback if requested
        if (i2c.callback!=0) i2c.callback();
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_WAIT_BUSY:
    ...
    case I2C_SM_STATE_WAIT_START:
        ...
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_WAIT_TX_ADDRESS:
        ...
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_WAIT_RX_ADDRESS:
        // check if address is set and acknowledged (EV6)
        ...
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_WAIT_TX_DATA:
        ...
        break;
    //-------------------------------------------------------
    case I2C_SM_STATE_WAIT_RX_DATA:
